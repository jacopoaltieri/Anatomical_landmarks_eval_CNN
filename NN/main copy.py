...
displacement_model_output = displacement_model.predict(train_images_dataset)


def custom_loss(laplacian_weight, displacement_model_output):
    def loss(y_true, y_pred):
        # Calculate the MSE between true and predicted images
        mse = tf.keras.losses.MeanSquaredError()(y_true, y_pred)

        # Calculate the Laplacian of the predicted displacement field using tfio.experimental.filter.laplacian
        laplacian = tfio.experimental.filter.laplacian(displacement_model_output,ksize=3)

        # Combine the MSE and Laplacian MSE using the Laplacian weight
        total_loss = mse + (laplacian_weight * laplacian)
...
# Define the Laplacian weight (you can adjust this value)
...
unet.compile(optimizer=adam, loss=custom_loss(laplacian_weight,displacement_model_output), metrics="mse")
...
plt.savefig("unet_deformation_example1.png")
plt.close()
...
# Adjust spacing between subplots
plt.tight_layout()
plt.savefig("tensor_deformation_example1.png")
plt.close()
...
# Adjust spacing between subplots
plt.tight_layout()
plt.savefig("inverse_deformation_example1.png")
plt.close()
print(f"Plot saved as {os.getcwd()}'/inverse_deformation_example1.png'")
...
plt.savefig('true_landmarks1.png')
plt.close()
...
plt.savefig('deformed_landmarks1.png')
plt.close()