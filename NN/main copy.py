...
def_image = tf.keras.layers.Lambda(apply_deformation)(
    [moving_image, displacement_tensor]
)
output = def_image
...
unet.compile(optimizer=adam, loss=custom_loss, metrics="mse")
...
deformed_image = tfa.image.dense_image_warp(
    test_image_expanded, -displacement_model_output
)
...
inverse_deformed = tfa.image.dense_image_warp(fixed_image_expanded, -inverse_transform)
...
def plot_with_landmarks_and_ROI(ax, image, deformed_landmarks, true_landmarks, bbox_size=15):
    ...
    for i in range(0, len(true_landmarks), 2):
        ...
        ax.plot(x, y, "go", markersize=2)  # Use "go" for green color

        # # Create a bounding box around the true landmark
        # bounding_box = Rectangle(
        #     (x - bbox_size / 2, y - bbox_size / 2),
        #     bbox_size,
        #     bbox_size,
        #     linewidth=1,
        #     edgecolor="g",  # Use "g" for green color
        #     facecolor="none",
        # )
        # ax.add_patch(bounding_box)
...
# Plot Deformed Moving Image
axs[0, 3].imshow(deformed_image[0], cmap="gray")
axs[0, 3].set_title("Deformed Moving")

# Plot Deformed Moving Image overlapped with Fixed Image
axs[1, 0].imshow(fixed_image, cmap="gray")
axs[1, 0].imshow(deformed_image[0], alpha=0.4)
axs[1, 0].set_title("Deformed Overlapping with Fixed")

# Plot Inverse Deformed Moving Image
axs[1, 1].imshow(inverse_deformed[0], cmap="gray")
axs[1, 1].set_title("Inverse Deformed Moving")

# Plot Inverse Deformed Moving Image overlapped with Moving Image
axs[1, 2].imshow(test_image, cmap="gray")
axs[1, 2].imshow(inverse_deformed[0], alpha=0.4)
axs[1, 2].set_title("Inverse Overlapping with Moving")

# Plot Moving Image with Landmarks and ROI
plot_with_landmarks_and_ROI(axs[1, 3], test_image, deformed_landmarks, test_labels)
axs[1, 3].set_title("Moving with Landmarks")
